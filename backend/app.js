require('dotenv').config(); // non-secret config: PORT, NODE_ENV, ALLOWED_ORIGINS, AWS_REGION, AWS_SECRET_NAME

const { loadSecrets } = require('./services/secretsManager');

(async () => {
  await loadSecrets(); // merges all secrets into process.env before any module reads them
